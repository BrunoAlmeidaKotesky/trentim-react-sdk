...
import {useRef, useState, useImperativeHandle, ForwardedRef, useEffect} from 'react';
...
function LifecycleProgressInner<StageData = any>(props: ILifecycleProgressProps<StageData>, ref:  ForwardedRef<ILifecycleProgressRef>) {
    ...
    const spanStyle = React.useCallback(() => props?.stageTextStyle ? 
        ...
        {fontSize: 14, padding: '0 12px 20px', cursor: 'pointer', color: '#333333'}, [props?.stageTextStyle]);
            ...
            <StagesColumn rightColPaddingBot={rightColPaddingBot} {...other} ref={stageColRef}>
                {props.stages.map((stage, index) => (
                    <StageBlock {...other}
                        ...
                        <span style={{...spanStyle(), fontWeight: stage?.active ? 600 : 'normal'}}>{stage?.label}</span>
                    </StageBlock>
                ))}
            </StagesColumn>
        </LifecycleContainer>
