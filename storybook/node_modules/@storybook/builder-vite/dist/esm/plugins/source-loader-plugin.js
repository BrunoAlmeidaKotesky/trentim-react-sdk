...
export function sourceLoaderPlugin(config) {
    ...
    return {
      ...
      async transform(src, id) {
        if (id.match(storyPattern)) {
          ...
          const s = new MagicString(src); // Entirely replace with new code
  ...
  return [{
    ...
    async transform(src, id) {
      if (id.match(storyPattern)) {
        let code = await sourceLoaderTransform.call(mockClassLoader(id), src); // eslint-disable-next-line @typescript-eslint/naming-convention

        const [_, sourceString] = code.match(storySourcePattern) ?? [null, null];

        if (sourceString) {
          const map = storySources.get(config);
          map?.set(id, sourceString); // Remove story source so that it is not processed by vite:define plugin
    ...
    async transform(src, id) {
      if (id.match(storyPattern)) {
        ...
        const map = storySources.get(config);
        const storySourceStatement = map?.get(id); // Put the previously-extracted source back in
